...
//     return intval(substr($version['VERSION'], 0, $start));
  ...
  public function getSizeInfo() {
    ...
    SELECT
        ...
        (CAST((SELECT  SUM(size) RowSize FROM sys.database_files WHERE type = 0 ) AS FLOAT)*8)/1024 RowSizeMB,
        (CAST((SELECT SUM(size) LogSize FROM sys.database_files WHERE type = 1 ) AS FLOAT)*8)/1024 LogSizeMB,
        (CAST((SELECT SUM(size) StreamSize FROM sys.database_files WHERE type = 2 ) AS FLOAT)*8)/1024 StreamSizeMB,
        (CAST((SELECT SUM(size) TextIndexSize FROM sys.database_files WHERE type = 4 ) AS FLOAT)*8)/1024 TextIndexSizeMB
